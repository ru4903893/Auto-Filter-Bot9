...
if n_offset == 0:
    ...
    links = ""
    for file_num, file in enumerate(files, start=offset+1):
        links += f"""<b>\n\n{file_num}. <a href=https://t.me/{temp.U_NAME}?start=file_{query.message.chat.id}_{file.file_id}>
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    elif query.data.startswith("rdxmovie_hd"):
...
async def auto_filter(client, msg, spoll=False):
    ...
    if imdb and imdb.get('poster'):
        ...
        if message.chat.id == SUPPORT_GROUP:
            buttons = [[InlineKeyboardButton('✧ ᴛᴀᴋᴇ ꜰɪʟᴇ ꜰʀᴏᴍ ʜᴇʀᴇ ✧', url="https://t.me/movie_search_group1")]]
